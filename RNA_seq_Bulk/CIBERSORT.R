...
CIBERSORT <- function(sig_matrix, mixture_file, perm=0, QN=TRUE, absolute=FALSE, abs_method='sig.score'){
    ...
    #read in data
X <- read.table("/work/morrissy_lab/Kiran/RESOURCES/Software/Cibersort/LM22.txt",header=T,sep="\t",row.names=1,check.names=F)
    #X <- sig_matrix
    Y <- mixture_file
Y <- read.table("RSEM_table_immunedeconv_inputFiles_human.txt", header=T,row.names=1, sep="\t",check.names=F)
